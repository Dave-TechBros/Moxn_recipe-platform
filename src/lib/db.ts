...
// Lazily ensure the schema and seed data exist on first query in production.
async function ensureDb(): Promise<void> {
  if (global._dbReady) return global._dbReady;
  global._dbReady = (async () => {
    // Apply schema (safe — uses IF NOT EXISTS).
    await pool.query(SCHEMA);

    // If recipes already exist, nothing more to do.
    ...
    // Seed 12 demo users with password "Password123".
    ...
    if (userIds.length === 0) return; // already had users

    // Lookup user id / category id helpers for recipes below.
    ...
    function uid(username: string) { return userMap[username]; }
    async function catId(slug: string) {
      const r = await pool.query("select id from categories where slug = $1", [slug]);
      return r.rows[0]?.id;
    }

    // Seed 20 starter recipes.
    interface RecipeSeed { a: string; c: string; title: string; summary: string; time: number; diff: string; ing: string[]; steps: string[]; img: string; feat?: boolean }
